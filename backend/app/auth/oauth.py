...
class KakaoOAuth(OAuthProvider):
    ...
    def get_authorization_url(self, state: Optional[str] = None) -> Tuple[str, str]:
        ...
        redirect_uri = f"{self.auth_config.backend_url}/auth/kakao/callback"

        params = {
            "client_id": self.auth_config.kakao_client_id,
            "redirect_uri": redirect_uri,
            "response_type": "code",
            "state": state
        }
    ...
    async def exchange_code_for_token(self, code: str) -> Dict:
        """
        Kakao Authorization Code를 Access Token으로 교환합니다.
        ...
        data = {
            "code": code,
            "client_id": self.auth_config.kakao_client_id,
            "client_secret": self.auth_config.kakao_client_secret,
            "redirect_uri": redirect_uri,
            "grant_type": "authorization_code"
        }
        ...
        try:
            ...
            response.raise_for_status()
            token_data = response.json()
            logger.info("[KakaoOAuth] 토큰 교환 성공")
            return token_data
        except httpx.HTTPError as e:
            logger.error(f"[KakaoOAuth] 토큰 교환 실패: {e}")
            raise
    ...
    async def get_user_info(self, access_token: str) -> Dict:
        ...
        try:
            ...
            user_info = {
                ...
                "email": kakao_account.get("email", f"kakao_{user_data['id']}@kakao.local"),
                "name": profile.get("nickname", "Kakao User"),
                "avatar_url": profile.get("profile_image_url")
            }

            logger.info(f"[KakaoOAuth] 사용자 정보 조회 성공: {user_info['email']}")
            return user_info
        except httpx.HTTPError as e:
            logger.error(f"[KakaoOAuth] 사용자 정보 조회 실패: {e}")
            raise
