...
> **변형 맥락 (중요)**: 이 Supervisor는 **variant A**(프로덕션 기본)에서 **규칙 기반(결정론적)** 으로 라우팅한다 — LLM으로 다음 에이전트를 고르지 **않는다**. LLM 슈퍼바이저 라우팅은 **variant A-hub**(`routing_mode="llm"`, `SUPERVISOR_LLM_ENABLED=true`)에서만 켜지는 측정 전용 경로다. M8 측정 결과 LLM 라우팅은 결정론 라우터와 100% 동일한 결정을 내리며 지연만 +74% 늘려, A는 결정론으로 동결됐다. 전체 비교: [변형 시스템 아키텍처](../../../docs/architecture/2026-07-05-variant-system-architecture.md), [A 결정 기록](../../../docs/architecture/2026-07-05-a-orchestration-decision.md).

## 1. 개요 (Overview)

MAS (Multi-Agent System) Supervisor는 DDOKSORI 소비자 분쟁 해결 챗봇의 중앙 오케스트레이션 레이어입니다. **LangGraph** 기반 **Hub-Spoke 아키텍처**로 구성되며, 전문 에이전트를 상태 기반 워크플로우 그래프를 통해 조율합니다. variant A에서 조율(라우팅)은 **결정론적**이며, LLM은 라우팅이 아니라 개별 에이전트(query_analysis 보조·answer_generation)에서만 쓰입니다.
...
- **라우팅**: variant A는 **결정론적 2-전략 라우팅**(규칙). LLM 라우팅 체인(GPT-4o -> Claude -> 규칙)은 variant A-hub에서만 활성화되는 측정 경로다.
...
### 라우팅 방식 (variant A vs A-hub)

**variant A (기본)**: 라우팅은 **100% 결정론적**이다. `decide_next_action` → `_deterministic_decision`이 `mode`와 완료 태스크에 따라 2-전략(Fast Path / Full Pipeline)으로 다음 노드를 규칙으로 정한다. **LLM을 라우팅에 호출하지 않는다.**

**variant A-hub (측정 전용)**: `routing_mode="llm"`일 때만 `_llm_routing_decision`이 아래 LLM 의사결정 체인을 사용한다:

```
1. Primary:   GPT-4o (SUPERVISOR_LLM_MODEL)             -- OpenAI
2. Fallback:  Claude 3.5 Sonnet (MODEL_SUPERVISOR_FALLBACK env)  -- Anthropic
3. Final:     규칙 기반 (_deterministic_decision)      -- LLM 실패/무효 시 폴백
```

즉 위 체인은 **A에서는 dead path**이고 A-hub에서만 동작한다. Primary가 타임아웃(30초)/파싱 불가 JSON/라우팅 불가 타깃을 반환하면 결정론 결정으로 폴백한다(M8은 이 폴백율·일치율을 측정). 상세: [M8 결과](../../../docs/plans/modules/M8-a-hub-routing-results.md).
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `SUPERVISOR_LLM_ENABLED` | `false` | **variant A-hub 전용**: LLM 슈퍼바이저 라우팅 활성화. A(기본)는 `false`로 결정론 유지 |
| `SUPERVISOR_LLM_MODEL` | `gpt-4o-mini` | A-hub 라우팅 LLM 모델 (graph_mas.py 래퍼). M8 측정은 gpt-4o 사용 |
...
| 2026-02-09 | - | 현재 코드베이스 상태를 반영하여 README 갱신. |
| 2026-07-05 | M8 | **variant A-hub 추가**(LLM 슈퍼바이저 라우팅 부활, `routing_mode`). A는 결정론으로 동결됨을 명확화. LLM 라우팅=A-hub 측정 전용(M8: 결정론과 100% 일치, 지연 +74%). |
