...
1.  **의도 분류 (Intent Classification)**: 질문 유형을 9가지 중 하나로 분류합니다 (아래 표 참조).
...
> **참고**: `NEED_USER_CLARIFICATION` 모드는 Phase 9에서 제거되었습니다. 모호한 쿼리는 `NEED_RAG`로 라우팅됩니다.
...
├── detectors.py             # 쿼리 패턴 감지 (도메인/모호함/시스템메타/메타대화)
...
├── tools.py                 # 유틸리티 함수 (search tool schemas)
└── docs/                    # 추가 문서
    ├── 01_PR_FastPath_Architecture.md
    ├── 02_PR_QueryAnalysis_Hybrid.md
    └── 03_LongTerm_FineTuning_Strategy.md
```
...
#### `classifier.py` - LLM 기반 분류기 (Issue #3)

| 클래스/함수 | 설명 |
|------------|------|
| `IntentClassifier` | gpt-4o-mini Function Calling 기반 분류기 |
| `HybridIntentClassifier` | Rule-based Fast Path + Redis Cache + LLM Fallback |
| `classify_intent(query)` | 편의 함수 (싱글톤 사용) |

#### `llm_classifier.py` - LLM Fallback (Issue #3)

| 함수 | 설명 |
|------|------|
| `llm_classify(query, timeout)` | gpt-4o-mini structured output으로 정확한 의도 분류 |
...
| `expand_query_for_law_search(...)` | 법령 검색 전용 쿼리 확장 (일상어 -> 법률 용어) |
| `expand_query_for_criteria_search(...)` | 분쟁해결기준 검색 전용 쿼리 확장 |
...
| `is_ambiguous_query(query)` | 하이브리드 모호 쿼리 감지 (Rule + LLM) |
...
| `is_followup_with_context(query, followups, threshold)` | 후속 질문 매칭 (SequenceMatcher) |

#### `extractors.py` - 정보 추출

| 함수 | 설명 |
|------|------|
| `normalize_query(query)` | 쿼리 정규화 + 대화 히스토리 방어 |
| `extract_keywords(query)` | 불용어 제거 + 동의어 정규화 키워드 추출 |
| `extract_info_from_message(query)` | 정규식 기반 엔티티 추출 (품목/날짜/금액) |
...
| `check_missing_onboarding_fields(...)` | 필수 정보 누락 확인 |
| `determine_agency_hint(query)` | 분쟁조정 기관 추천 (KCA/ECMC) |
...
| 함수 | 설명 |
|------|------|
| `expand_query_by_type(query, type, ...)` | v1 규칙 기반 유형별 확장 |
| `generate_search_queries(original, expanded, keywords)` | v1 Multi-Query Expansion (최대 4개) |
| `expand_query_with_llm_v2(query, keywords, intent)` | v2 LLM 기반 확장 래퍼 (최대 5개) |
...
**분류 우선순위** (classifiers.py):

```
1.  system_meta (0.95) - "너 누구야?" 패턴
2.  general (0.90) - 인사/감사 패턴
3.  law (0.90) - 법률명 패턴 (\S+법)
4.  general/definitional (0.85) - "환불이 뭐예요?" 패턴
5.  restricted (0.90) - 전문기관 도메인 키워드
6.  procedure (0.85) - 절차 안내 키워드
7.  law (0.80) - 법령 키워드 2개 이상
8.  criteria (0.80~0.90) - 기준 키워드 + 제품명
9.  meta_conversational (0.90) - "뭘 물어봐야 할까?" 패턴
10. dispute compound (0.80~0.85) - 복합 분쟁 패턴
11. ambiguous (0.60) - 하이브리드 모호 감지
...
class IntentClassificationResult:
    ...
    domain: Optional[RestrictedDomain]
...
test_classifier.py::test_system_meta_detection PASSED      # 시스템 메타 감지
test_new_query_types.py::test_meta_conversational PASSED   # 메타 대화형 감지
...
```
...
| 2026-01-27 | **Phase 8** | Query Rewriter 모듈 아카이브. QueryAnalysisAgent가 쿼리 확장 전담. |
| 2026-01-28 | **Phase 9** | **Clarification 로직 제거**. `NEED_USER_CLARIFICATION` 모드 삭제. `needs_clarification` 파라미터는 backward compatibility 유지하나 무시됨. 모호 쿼리는 `NEED_RAG`로 라우팅. |
...
| 2026-01-28 | **PR-7** | General 채팅에서 분쟁 의도 키워드 Safety Net 추가. |
