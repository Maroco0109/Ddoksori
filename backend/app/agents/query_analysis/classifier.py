...
QueryType = Literal[
    'dispute', 'law', 'criteria', 'procedure',
    'restricted', 'general', 'system_meta', 'ambiguous'
]
    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                "query_type": {
                    "type": "string",
                    "enum": ["dispute", "law", "criteria", "procedure",
                             "restricted", "general", "system_meta", "ambiguous"],
                    "description": "분류된 쿼리 유형"
                },
                "domain": {
                    "type": ["string", "null"],
                    "enum": ["consumer", "finance", "medical", "privacy",
                             "realestate", "construction", None],
...
class IntentClassifier:
    ...
    def _log_classification(self, query: str, result: Dict[str, Any]):
        ...
        logger.info(f"[IntentClassifier] Classification: {json.dumps(log_entry, ensure_ascii=False)}")
...
class HybridIntentClassifier:
    ...
    def __init__(
        ...
        self._system_meta_patterns = [
            "너 누구", "네 이름", "뭐야 너", "누가 만들",
            "어떤 모델", "무슨 ai", "무슨 llm", "기능이 뭐",
            "뭘 할 수", "뭐 할 수", "어떤 일을",
        ]
    ...
    def classify(
        ...
        if len(query_lower) <= 10 and any(p in query_lower for p in self._greeting_patterns):
    ...
    def _save_to_cache(self, query: str, result: IntentClassificationResult) -> None:
        ...
        try:
            from app.supervisor.cache import IntentClassificationCache
            IntentClassificationCache.set(query, {
                'query_type': result.query_type,
...
def classify_intent(query: str, context: Optional[Dict[str, Any]] = None) -> IntentClassificationResult:
    """편의 함수: 쿼리 의도 분류"""
