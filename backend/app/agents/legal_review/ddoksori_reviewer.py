...
class DdoksoriReviewer:
    ...
    def __init__(self):
        ...
        self.forbidden_headers = [
            ...
            "[법률]", "[해결기준]", "[유사사례]", "[상황 공감]", 
            "[위로와 전환]", "[사건 요약 정리]", "[논리적 근거 안내]", 
            "[행정 절차 안내]", "[전문 기관 연결]", "[근거 및 이유 안내]"
        ]
        
        # [v2.4.1 업데이트] 할루시네이션 체크에서 제외할 화이트리스트 및 허용 지침
        self.whitelist = [
            # 주요 공적 기관 및 번호
            "한국소비자원", "대한법률구조공단", "경찰청", "경찰서", "1372 소비자상담센터",
            "소비자분쟁조정위원회", "전자거래분쟁조정위원회", "콘텐츠분쟁조정위원회", 
            "1372", "132", "112", "110",
            
            # [허용] 데이터에 구체적 숫자가 없을 때 사용하는 추상적 수치 표현
            "일정 비율", "관련 기준", "정당한 보상", "해당 규정", "일정 금액", "일부 금액",
            
            # 법적 절차 및 문서
            "내용증명", "민사소송", "소액심판", "지급명령", "소장", "증거자료", 
            "계약서", "영수증", "이체내역", "배달증명", "피해구제", "분쟁조정",
            
            # 템플릿 고정 문구
            "보험이자 증거 자료", "든든한 보험", "정당한 주장을 기록", "소중한 자료",
            "상담 예약", "피해 사실 요약", "사건 요약"
        ]
...
"""
        res = self.client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "데이터의 근거와 템플릿 지침을 구분할 줄 아는 합리적이고 정확한 감사관입니다."},
