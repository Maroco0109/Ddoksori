...
"""
    ...
    def _extract_claim_evidence_map(
        ...
        chunk_map = {c.get('chunk_id', c.get('unit_id', f'chunk_{i}')): c for i, c in enumerate(all_chunks)}
        ...
        for sentence in sentences:
            ...
            for chunk_id, chunk in chunk_map.items():
                content = chunk.get('content', chunk.get('text', chunk.get('unit_text', '')))
                if not content:
                    continue
                ...
                key_terms = ['소비자', '분쟁', '환불', '조정', '법', '기준', '신청', '피해', '배상']
                key_match = sum(1 for term in key_terms if term in sentence_lower and term in content_lower)
