...
def verify_raw_sql():
    ...
    try:
        cur.execute("SELECT doc_id, title, content FROM documents WHERE title ILIKE %s LIMIT 5", (f"%{target_title_part}%",))
        docs = cur.fetchall()
    ...
    try:
        ...
        cur.execute("SELECT chunk_id, text, metadata FROM vector_chunks WHERE metadata->>'title' ILIKE %s LIMIT 50", (f"%{target_title_part}%",))
        chunks = cur.fetchall()
