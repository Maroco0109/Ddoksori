...
from app.supervisor.graph import summarize_node_output, build_pipeline_summary


@pytest.mark.unit
def test_trace_entry_fields():
    """TraceEntry TypedDict가 정확히 5개의 필드를 가지는지 확인"""
    expected_fields = {'node_name', 'timestamp', 'duration_ms', 'protocol_summary', 'metadata'}
...
def test_summarize_node_output_cache_check():
    ...
    assert summarize_node_output('cache_check', {'_cache_hit': True}) == {'cache_hit': True}
    assert summarize_node_output('cache_check', {'_cache_hit': False}) == {'cache_hit': False}
...
def test_build_pipeline_summary():
    ...
    entries = [
        {'node_name': 'cache_check', 'timestamp': 3.0, 'duration_ms': 50.0, 'protocol_summary': {'cache_hit': False}, 'metadata': None},
        {'node_name': 'supervisor', 'timestamp': 1.0, 'duration_ms': 100.0, 'protocol_summary': {'current_phase': 'init'}, 'metadata': None},
        {'node_name': 'query_analysis', 'timestamp': 2.0, 'duration_ms': 75.0, 'protocol_summary': {'intent': 'general'}, 'metadata': None},
    ]
    ...
    assert per_node[0] == {'seq': 0, 'node': 'supervisor', 'duration_ms': 100.0, 'summary': {'current_phase': 'init'}}
    assert per_node[1] == {'seq': 1, 'node': 'query_analysis', 'duration_ms': 75.0, 'summary': {'intent': 'general'}}
    assert per_node[2] == {'seq': 2, 'node': 'cache_check', 'duration_ms': 50.0, 'summary': {'cache_hit': False}}
...
def test_trace_timestamp_ordering():
    ...
    entries = [
        {'node_name': 'a', 'timestamp': 3.0, 'duration_ms': 10.0, 'protocol_summary': None, 'metadata': None},
        {'node_name': 'b', 'timestamp': 1.0, 'duration_ms': 10.0, 'protocol_summary': None, 'metadata': None},
        {'node_name': 'c', 'timestamp': 2.0, 'duration_ms': 10.0, 'protocol_summary': None, 'metadata': None},
    ]
...
def test_trace_operator_add_merge():
    ...
    e1: TraceEntry = {'node_name': 'retrieval_law', 'timestamp': 1.0, 'duration_ms': 50.0, 'protocol_summary': {'source': 'law'}, 'metadata': None}
    e2: TraceEntry = {'node_name': 'retrieval_criteria', 'timestamp': 1.1, 'duration_ms': 60.0, 'protocol_summary': {'source': 'criteria'}, 'metadata': None}
    e3: TraceEntry = {'node_name': 'retrieval_case', 'timestamp': 1.2, 'duration_ms': 70.0, 'protocol_summary': {'source': 'case'}, 'metadata': None}
    ...
    assert summary['node_count'] == 3
    assert summary['node_sequence'] == ['retrieval_law', 'retrieval_criteria', 'retrieval_case']
