...
class TestConversationMemoryWithDB:
    ...
    def test_memory_initialization_with_new_conversation(self, mock_conversation_db):
        ...
        async def _test():
            ...
            mock_conversation_db.get_conversation_by_session.assert_called_once_with('sess_123')
            mock_conversation_db.create_conversation.assert_called_once()

        run_async(_test())

    def test_memory_initialization_with_existing_conversation(self, mock_conversation_db):
    ...
    def test_db_failure_falls_back_to_memory(self, mock_conversation_db):
        ...
        async def _test():
            mock_conversation_db.get_conversation_by_session.side_effect = Exception('DB connection failed')
