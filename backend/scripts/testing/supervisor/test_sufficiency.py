...
class TestSufficientResult:
    ...
    def test_sufficient_confidence_above_threshold(self, checker, sufficient_retrieval_result):
...
class TestInsufficientResult:
    ...
    def test_insufficient_clarifying_questions(self, checker, insufficient_retrieval_result):
        """불충분 시 구체적인 질문 3개 포함"""
        result = checker.evaluate(insufficient_retrieval_result)
    ...
    def test_insufficient_confidence_below_threshold(self, checker, insufficient_retrieval_result):
...
class TestEnvThresholdOverride:
    ...
    def test_custom_thresholds_affect_result(self, monkeypatch, partial_retrieval_result):
        """
...
class TestReasonGeneration:
    ...
    def test_insufficient_reason_includes_issues(self, checker, insufficient_retrieval_result):
        """불충분한 경우 구체적인 문제점 나열"""
        result = checker.evaluate(insufficient_retrieval_result)
