...
class TestPIIMasker:
    ...
    def test_mask_multiple_pii(self):
        text = "010-1234-5678로 test@example.com 또는 서울시 강남구 역삼동으로 연락주세요"
        masked = PIIMasker.mask_text(text)
...
class TestDataCollectorGeneration:
    ...
    def test_generate_training_examples_pii_masking(self, tmp_path):
        ...
        assert all('[PHONE]' in ex.input for ex in examples if '010-1234-5678' in qa_data['user_query'])
        assert any('[EMAIL]' in ex.output for ex in examples if 'test@example.com' in qa_data['rewritten_query'])
