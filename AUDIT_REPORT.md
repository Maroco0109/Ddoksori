...
**심각도**: High
**상태**: ✅ Closed (해결됨)
**현재 상태 (2026-02-04)**: ✅ 해결됨 — 17개 활성 문서 파일 마스킹 완료

**영향받는 파일** (마스킹 완료):
- `docs/guides/deployment-troubleshooting.md` - JWT 시크릿, RDS 엔드포인트
- `docs/guides/deployment-execution-guide.md` - JWT 시크릿, 개발자 경로
- `docs/data/db/03_04_DB연결및테스트안내.md` - RDS 엔드포인트 (11개 위치)
- `docs/data/db/03_03_DB_n_API결과.md` - RDS 엔드포인트 (6개 위치)
- `docs/guides/cicd-setup-guide.md` - IP 주소, 프로덕션 경로
- `docs/feature/conversational-chatbot-*.md` - RDS 엔드포인트, 개발자 경로
- `docs/data/law_guide/03_02_임베딩결과.md` - Windows 사용자명
- `docs/data/db/01_01_DB구축방법.md` - Windows 사용자명
- 외 9개 파일

**설명**:

보안 감사 결과, 프로젝트 문서에 다음 민감 정보가 평문으로 노출되어 있음을 발견:
- JWT_SECRET_KEY 평문 노출 (2개 파일, 3개 위치)
- RDS 엔드포인트 노출 (4개 파일, 40+ 위치)
- 공인 IP 주소 노출 (1개 파일)
- 개발자/프로덕션 서버 경로 노출 (8개 파일, 40+ 위치)
- Windows 사용자명 노출 (3개 파일)

**위험**:

- JWT 시크릿 노출 시 토큰 위조 가능
- RDS 엔드포인트 노출 시 데이터베이스 타겟 공격 가능
- 개발자 경로 노출 시 시스템 구조 파악 용이

**권장 조치** (완료됨):

모든 민감 정보를 예시값으로 마스킹 처리:
- RDS: `your-db-instance.ap-northeast-2.rds.amazonaws.com`
- JWT: `your-jwt-secret-key-min-32-characters`
- IP: `xxx.xxx.xxx.xxx`
- 경로: `/path/to/project/`

**수락 기준** (충족됨):

- ✅ `grep -r "cyhiie0gambz"` 결과 0건 (활성 문서)
- ✅ `grep -r "yQ7XvZ3mN8kP2wR5tL9xU6bC4aJ1sH0e"` 결과 0건
- ✅ `grep -r "15\.165\.215\.141"` 결과 0건
- ✅ 개발자 경로 마스킹 완료
- ✅ 문서 가독성 및 예시로서의 유용성 유지
